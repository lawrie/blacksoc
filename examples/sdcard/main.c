...
int main()
{
	...
	printf("Writing 0xFF pattern to blocks 0 and 1..\n");

	for (int i = 0; i < 512; i++)
		buffer[i] = 0xff;

	sdcard_write(buffer, 0);
	...
	printf("Writing count up pattern to block 0..\n");

	for (int i = 0; i < 512; i++)
		buffer[i] = i;
	sdcard_write(buffer, 0);

	printf("Writing count down pattern to block 1..\n");
